library(mizer)
params_knife <- set_community_model(z0 = 0.1, recruitment = 4e7, alpha = 0.2, f0 = 0.7, knife_edge_size = 1000, sigma = 3, h=10, beta = 100, zeta = 4000, n=0.88)


simNoFishing <- project(params_knife, effort = 0, t_max = 200, dt=0.01)
...
#Discard the first 49 time steps as this is used by the system to find the fixed point of cycle.
time = c(49:(length(plankton_no_fishing[,1])-1))
y1 = ((log(plankton_no_fishing[,planton_index]) - log(plankton_no_fishing[50,planton_index])))[-1:-49]
...
data <- data.frame(time, Init_Plankton, y1, y2, y3, y4)


#Plots the "size-mograph"
